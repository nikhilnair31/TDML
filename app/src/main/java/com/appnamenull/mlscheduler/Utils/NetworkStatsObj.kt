...
class NetworkStatsObj{
    ...
    fun getAllRxBytesMobile(context: Context): Long {
        val bucket: NetworkStats.Bucket = try {
            networkStatsManager.querySummaryForDevice( ConnectivityManager.TYPE_MOBILE, getSubscriberId(context, ConnectivityManager.TYPE_MOBILE), 0, System.currentTimeMillis() )
        }
    ...
    val allRxBytesWifi: Long
        get() {
            val bucket: NetworkStats.Bucket = try {
                networkStatsManager.querySummaryForDevice(  ConnectivityManager.TYPE_WIFI, "", 0, System.currentTimeMillis() )
            }
            catch (e: RemoteException) {
                return -1
            }
    ...
    fun getPackageRxBytesMobile(context: Context): Long {
        ...
        networkStats = try {
            networkStatsManager.queryDetailsForUid( ConnectivityManager.TYPE_MOBILE,  getSubscriberId(context, ConnectivityManager.TYPE_MOBILE), 0, System.currentTimeMillis(), packageUid )
        } catch (e: RemoteException) {
            return -1
        }
        var rxBytes = 0L
    ...
    val packageRxBytesWifi: Long
        get() {
            var networkStats: NetworkStats? = null
            networkStats = try {
                networkStatsManager.queryDetailsForUid( ConnectivityManager.TYPE_WIFI, "", 0, System.currentTimeMillis(), packageUid )
            }
            catch (e: RemoteException) {
                return -1
            }
